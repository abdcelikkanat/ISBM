...
class CGS_IRM:
    def __init__(
        self,
        alpha=1.0,  # hyperparameter of CRP for z_1 (eq. 7)
        a_intra = 10.0,  # Hyperparameter of the beta distribution (eq. 6)
        a_inter = 1.0,
        b_intra = 1.0,
        b_inter = 1.0, # Hyperparameter of the beta distribution (eq. 6)
        n_iter = 500,  # Number of iterations
        burnin = 250,  # The sweeps discarded before collecting samples (To allow the Markov chain to reach convergence)
        seed = None,
    ):
        self.alpha = alpha
        self.a_intra, self.a_inter = a_intra, a_inter
        self.b_intra, self.b_inter = b_intra, b_inter
        self.n_iter = n_iter
        self.burnin = burnin
        self.seed = seed
    ...
    def fit(self, X):
        ...
        X = np.asarray(X, dtype=np.float64)  # convert input matrix X to a 2D array of floats
        N = X.shape[0] # Extrtact dimensions. N1 is the objects in the first domain, and N2 is the objects in the second domain.

        # singleton initialization of
        z = np.arange(N, dtype=int)
        K = N  # Upper bound of clusters
        ...
        for i in range(N):
            for j in range(N):
                n[z[i], z[j]] += X[i, j]
                Nm[z[i], z[j]] += 1.0 - X[i, j]

        self.z_samples_ = []  # append accepted cluster assignments
        ...
        for it in range(self.n_iter):
            for i in rng.permutation(N):
                ...
                m[k_old] -= 1
                np.add.at(n[k_old], z, -X[i, :])
                np.add.at(Nm[k_old], z, -(1.0 - X[i, :]))

                n_plus = np.zeros(K, dtype=float)
                N_plus = np.zeros(K, dtype=float)
                np.add.at(n_plus, z, X[i, :])
                np.add.at(N_plus, z, 1.0 - X[i, :])

                active = np.where(m > 0)[0]

                existing = np.where(m > 0)[0]
                log_p = []
                ids = []
                for k in existing:
                    nk = n[k, active]
                ...
                np_ = n_plus[active]
                Np_ = N_plus[active]
                lp_new = np.log(self.alpha) + np.sum(
                    gammaln(self.a + np_)
                    + gammaln(self.b + Np_)
